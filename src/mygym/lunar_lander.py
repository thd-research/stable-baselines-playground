...
def cpp_safe_input(*args, scalar=False):
    """
    Converts all inputs to native Python types.
    """

    if len(args) == 1 and np.isscalar(args[0]):
        return args[0].item()
    
    safe_args = [arg.item() if isinstance(arg, (np.generic, np.ndarray)) else arg for arg in args]
    return safe_args
    ...
    def step(self, action):
        ...
        if (self.continuous and action[0] > 0.0) or (
            ...
            self.lander.ApplyLinearImpulse(
                cpp_safe_input(-ox * MAIN_ENGINE_POWER * m_power, -oy * MAIN_ENGINE_POWER * m_power),
                impulse_pos,
                True,
            )
        ...
        if (self.continuous and np.abs(action[1]) > 0.5) or (
            ...
            impulse_pos = cpp_safe_input(
                self.lander.position[0] + ox - tip[0] * 17 / SCALE,
                self.lander.position[1] + oy + tip[1] * SIDE_ENGINE_HEIGHT / SCALE,
            )
            if self.render_mode is not None:
                # particles are just a decoration, with no impact on the physics, so don't add them when not rendering
                p = self._create_particle(0.7, 
                                          impulse_pos[0], 
                                          impulse_pos[1], 
                                          s_power)
                p.ApplyLinearImpulse(
            ...
            self.lander.ApplyLinearImpulse(
                cpp_safe_input(-ox * SIDE_ENGINE_POWER * s_power, -oy * SIDE_ENGINE_POWER * s_power),
                impulse_pos,
                True,
            )
